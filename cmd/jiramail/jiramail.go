...
This utility synchronizes the state of jira with local maildir and
provides SMTP transport for jira.
...
func main() {
	...
	if len(cfg.Core.LockDir) > 0 {
		...
		lockDirs = append(lockDirs, cfg.Core.LockDir)
	}
	...
	if cfg.SMTP != nil {
		if len(cfg.SMTP.LockDir) > 0 {
			...
			if err != nil {
				if !os.IsExist(err) {
					logrus.Errorf("smtp: unable to create lock directory: %s", err)
				}
				logrus.Debugf("smtp: lock directory is already exists: %s", cfg.SMTP.LockDir)
				return
			}
			lockDirs = append(lockDirs, cfg.SMTP.LockDir)
		}
